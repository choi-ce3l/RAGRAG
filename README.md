...
이 저장소 루트가 곧 QA 서비스 본체다 — `qa/`, `server.py` 등이 레포 루트에 바로 있다.
다만 **형제 디렉토리 `code_chunkingandparsing/`이 반드시 이 레포 루트 아래 있어야** 동작한다 —
`qa/*.py`가 그 폴더를 상대 경로로 참조한다(예: `qa/kg.py`의 `import numqa`).
...
├── data/                      런타임 데이터 — 원본+캐시 (아래 "데이터" 참고)
├── Dockerfile
├── requirements.txt
├── .env.example
├── README.md                  ← 이 파일
│
├── code_chunkingandparsing/   ← 필수 의존성 (numqa 등 파싱·팩트 추출 코드)
│   ├── src/                   qa/가 import — 실행 필수
│   └── out/                   원본 팩트 데이터(factstore.jsonl 등) — 별도 다운로드
│
└── ragrag/                    ← 별도 트랙: 검색 랭킹·지표 온톨로지 확장 파이프라인
                                  (이 QA 서비스가 쓰는 metric 사전을 확장하는 작업 중인 코드,
                                   서빙 경로엔 필수 아님)
```
...
경로를 못 찾는다. `data/`도 마찬가지로 `corpus/manifest.jsonl`(원본)과 `*.jsonl`(캐시)을
한 폴더에 같이 두고 `/app/data`에 통째로 마운트한다.
